...
async def channel_post(client: Client, message: Message):
    reply_text = await message.reply_text("Sabar dulu cok...!", quote = True)
    try:
        ...
        print(e)
        await reply_text.edit_text("Sepertinya ada kesalahan..!")
        return
...
async def new_post(client: Client, message: Message):
    ...
    base64_string = await encode(string)
    link = f"https://t.me/{client.username}?start={base64_string}"
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("🔁 Share ke temen yg otak bokp", url=f'https://telegram.me/share/url?url={link}')]])
    try:
